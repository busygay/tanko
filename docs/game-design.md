...
- ✅ **过载机制**：实现了 `accumulatedDamage` 记录，受击时通过 `BoomColorStart` 变红闪烁（过载视觉反馈）。
...
| **行为模式** | 1. **待机 (Idle):** 在空中悬浮，触手摆动<br>2. **移动 (Walk):** 生成后缓慢飘向玩家，保持适当距离<br>3. **攻击 (Attack):** 进入射程后持续发射资源吞噬光束 | 飞行单位行为，专注于资源干扰和掠夺 |
...
| **行为模式** | 1. **待机 (Idle):** 站立不动，手中法杖散发腐化能量<br>2. **移动 (Walk):** 生成后缓慢移动，保持与玩家的安全距离<br>3. **攻击 (Attack):** 到达适当位置后，为友方单位提供灵魂链接支援 | 辅助型单位行为，专注于为友军提供保护和支援 |
...
- **功能**：通用的Buff管理框架，支持Buff的应用、叠加、持续时间更新和移除。
- **应用实例**：白骨图腾提供的加速Buff。
...
- **状态定义**：
    ...
    - `HURT`: 受击硬直
    - `DEATH`: 死亡结算与动画
- **优势**：确保了敌人行为逻辑的清晰和易扩展性（如哥布林工兵的建造模式即扩展了此状态机）。
