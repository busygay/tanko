...
| **N2** | 1000词 | 中高级 | 复杂词汇，接近母语水平 | ✅ 已实现基础架构 |
| **N1** | 1500词 | 高级 | 专业词汇，达到熟练运用 | ✅ 已实现基础架构 |

**总计：4300个词汇**

**实现状态分析：**
- ✅ **基础架构支持**：系统已支持动态加载任意CSV格式题库，无需修改代码即可扩展N2/N1。
- ❌ **数据缺失**：N2 and N1 级别的 CSV 文件尚未制作和配置。
...
- ✅ UI界面：在menu界面实现题库选择
- ✅ 多级别组合：可以同时选择多个题库
- ✅ **动态耗尽处理**：当当前题库题目用尽时，游戏会自动暂停并弹出选择界面，允许玩家动态加载其他题库继续游戏。
...
- ✅ 错误答案生成包含混淆词
- ✅ 4选1格式正确实现
- ✅ **跳过机制**：在[`answering.gd:132-137`](main/UI/answering.gd:132)中实现，跳过题目时不获得/损失资源，不触发技能，不影响连击和错题记录
- ✅ **跳过信号**：`questionSkipped`信号在[`eventmanger.gd:94`](global/eventmanger.gd:94)中定义并正确触发
...
- ✅ **备份干扰项**：引入了 `BACKUP_KANA_CHARS` 机制，当候选干扰词不足时，自动使用预设假名填充，确保题目完整性。
- ✅ 触发机制：已实现每5道题后的随机判定（50%概率）
- ✅ 完成奖励机制：已实现额外AP奖励
- ✅ 答题机制：已实现逐个字符串检查
- ✅ **AP额外奖励**：单词重组完成后，在[`main.gd:94-106`](main/main.gd:94)中实现额外奖励1AP的逻辑
...
- ✅ 基础选择题机制：复用基础选择题进行答题
- ✅ 错题本检查：当错题本为空时自动跳过
- ✅ **历史分离**：支持当前游戏错题与历史累计错题的分离管理与合并。
- ✅ **已掌握单词系统**：`masteredWord`字典用于存储错误次数为0且从错题本移除的单词，支持从文件加载和保存（见[`jlptn_5.gd:184-249`](global/jlptn_5.gd:184)）
...
- ✅ 跨游戏累积：已实现错误次数的跨会话累加
- ✅ 数据格式：已使用JSON格式存储
- ✅ **ID映射系统**：使用全局`_id_map`实现基于ID的单词引用，支持词库更新后的数据一致性维护
...
**实现状态：✅ 已完成**
- ✅ **界面实现**：`word_confirm.gd` 已实现完整的错题展示界面。
- ✅ **准确率统计**：界面包含实时准确率计算（正确数/总题数）并以百分比显示。
- ✅ **交互功能**：支持查看正确/错误列表，以及“下一关”跳转功能。
...
- ⚠️ **命名差异说明**：代码实际实现中，**EP (能量点)** 对应变量名为 `Correctcount`，**AP (行动点)** 对应变量名为 `power`。这种命名差异不影响功能逻辑。
...
| 枪斗术 | 弹幕主宰 | 一枪双击和QQ弹技能满级 | 一次射击发射 6 发子弹。触发 Five Combo 时，子弹数量翻 5 倍（30发），且跳弹次数提升至 10 次。 | **特殊技能**：直接满级，不参与常规升级。强力爆发技能。 |
...
| 三重协议 | BT-7270 UP | BT-7270 | (被动) BT-7270 炮塔伤害提升 | 每次触发永久提升 BT-7270 炮塔 5% 的伤害输出。 |
| 三重协议 | BT-7271 | 无 | 召唤自动寻敌机器人BT-7271协助战斗 | 具有自动移动、近战攻击和雷霆标记能力的强化单位。 |

**实现状态：✅ 已完成（包含3D渲染和粒子效果）**
- ✅ **BT-7270**：[`BT-7270.gd`](skill/scene/BT-7270.gd:9)已完整实现
- ✅ **BT-7270 UP**：[`bt-7270_up.gd`](skill/scene/bt-7270_up.gd:1)已实现，提供每层 5% 的永久伤害加成。
- ✅ **BT-7271**：[`BT-7271.gd`](skill/scene/BT-7271.gd:1)已实现召唤逻辑，[`robot.gd`](globalSkillData/robot.gd:1)实现了完整的寻敌、移动、攻击和雷霆标记系统。
- ✅ **BT-7271 UP**：[`bt-7271_up.tscn`](skill/scene/bt-7271_up.tscn)已实现，每次升级永久提升机器人5%的移动速度（[`get_speed_multiplier`](skill/scene/BT-7271.gd:89)）
- ✅ **3D渲染系统**：使用 `SubViewport` 实现了3D模型在2D场景中的渲染
  - **坐标投射**：通过 `Camera3D.unproject_position()` 实现3D到2D坐标转换
  - **实时同步**：3D模型位置与2D碰撞检测和粒子效果实时同步
  - **瞄准机制**：2D角度计算后应用于3D模型Y轴旋转
...
| 系列名 | 技能名 | 前置技能 | 内容（主动技能） | 关键机制与优化点 |
| :--- | :--- | :--- | :--- | :--- |
| 战术投掷 | 燃烧瓶 (Molotov) | 无 | 投掷燃烧瓶，根据触发器不同产生不同效果 | **多模式触发**：根据不同触发器产生不同布局 |
| 战术投掷 | 地雷 (Landmine) | 无 | 投掷地雷，根据触发器不同产生不同效果 | **可升级技能**：每级提升20px爆炸半径 |

**实现状态：✅ 已完成**
- ✅ **燃烧瓶**：[`molotov.gd`](skill/scene/molotov.gd:1)已完整实现多模式触发
  - **fiveCombo（金黄色长效火墙）**：生成5个火瓶形成火墙，横向排列
  - **brokenCombo（环形火焰保护圈）**：在玩家周围半径150px环形部署5个火瓶
  - **APSpent（高能蓝火）**：追踪敌人生成火瓶，优先锁定敌人
  - **默认（火海）**：随机位置生成火瓶
- ✅ **地雷**：[`landmine.gd`](skill/scene/landmine.gd:1)已完整实现多模式触发
  - **fiveCombo（奖励分支）**：投掷5个地雷，每个投掷到敌人前进方向+50px偏移位置
  - **brokenCombo（生存分支）**：在玩家防线最内侧随机部署3枚高压地雷（伤害×1.5）
  - **APGained（资源分支/精英预判）**：优先攻击精英敌人，无精英时攻击血最厚的敌人
  - **默认**：投掷到随机敌人位置的前进方向+50px偏移位置
  - **升级机制**：每级提升20px爆炸半径（[`base_explosion_radius`](skill/scene/landmine.gd:4)=100px）
- ✅ **智能投掷**：地雷使用[`_get_enemy_forward_pos`](skill/scene/landmine.gd:13)方法获取敌人前进方向
- ✅ **地雷实体**：[`mine_entity.gd`](globalSkillData/mine_entity.gd:1)实现地雷爆炸效果
- ✅ **燃烧瓶实体**：[`molotov_entity.gd`](globalSkillData/molotov_entity.gd:1)实现燃烧瓶效果
...
- ✅ 伤害计算：基于炮塔自身[`damage`](globalSkillData/turret-bt-7270.gd:24)变量，初始为0，替换旧炮塔时继承20%伤害+[`damageMultiplier`](skill/scene/BT-7270.gd:8)加成
- ✅ 射击模式：4颗子弹，0.15s间隔，0.5s冷却
- ✅ 索敌系统：[`rotationSpeed`](globalSkillData/turret-bt-7270.gd:30)为π/2（约1.57秒一圈），自动瞄准
- ✅ 状态管理：spawn、idle、aim、att、die状态完整实现
- ✅ **视觉表现**：完整的3D模型渲染、旋转动画及射击粒子效果
- ✅ 音效系统：射击音效播放
- ✅ **炮塔替换**：新炮塔生成时继承旧炮塔伤害的20%+加成倍率，旧炮塔播放破碎动画后销毁

#### 2. BT-7271 (机器人)
BT-7271 是 BT-7270 的强化进阶版，具有移动能力和更复杂的战术价值。

| 属性/机制 | 数值/说明 | 备注 |
| :--- | :--- | :--- |
| 存活时间 | 15 秒 | |
| 移动速度 | 100 px/s (普通) / 125 px/s (强化触发) | |
| 攻击范围 | 100 像素 (近战) | |
| 核心机制 | **雷霆标记**：攻击命中时施加。持续 5s，使目标减速 30%，受到的伤害增加 25%。 | 战略核心：优先攻击未标记的目标。 |
| 特殊能力 | **雷霆脉冲**：(待触发/条件触发) 对半径 200px 内敌人造成 AOE 伤害，并对标记敌人造成额外 25% 伤害。 | 伴随雷电锁链粒子效果。 |

**实现状态：✅ 已完成**
- ✅ 移动寻敌：完整的 AStar/直线 寻敌逻辑，自动锁定最近且未被标记的敌人。
- ✅ 状态机管理：包含 spawn, idle, walk, att, jumpAtt, death 状态。
- ✅ 雷霆系统：与 Buff 系统集成，成功实现减速和伤害加深逻辑。
- ✅ 视觉效果：3D 模型渲染，支持多种动画状态，包含攻击命中检测信号。
...
效果：加速光环。自身半径 200px 范围内的所有敌人获得 50% 速度加成（speed_multiplier: 1.5）。

**实现状态：✅ 已实现**
- ✅ **生成机制**：由哥布林工兵在建造者模式下献祭生成
- ✅ **功能实现**：[`totem.gd`](globalSkillData/totem.gd:1)实现了对范围内敌人的检测
- ✅ **效果实现**：为范围内敌人施加持续的加速Buff（[`speed_multiplier`](globalSkillData/totem.gd:4)默认1.5倍速度）
- ✅ **视觉反馈**：为被加速的敌人添加绿色Buff视觉效果（[`speed_buff.tscn`](main/speed_buff.tscn)）
- ✅ **Buff管理**：完整的Buff应用、持续、移除流程
- ✅ **敌人Buff系统**：与[`base_enemy.gd`](enemy/base_enemy.gd:8)的Buff系统完美集成
...
| **普通敌人数量** | `(当前关卡数 - 1) * 3 + 5` | 线性增长，确保基础挑战难度稳步提升。实际使用`(L-1)*3+5`公式 |
...
- ✅ 悬浮效果：通过动画 and 位置控制实现上下浮动
- ✅ 火球生成：[`fly_demon.gd:26`](enemy/fly_demon.gd:26)中完整实现火球生成 and 发射
- ✅ 目标选择：智能选择最近的玩家目标
- ✅ 攻击间隔：10秒攻击间隔
...
- ✅ 干扰机制：玩家[`player.gd:379-383`](main/player.gd:379)中实现EP干扰逻辑
- ✅ 层数管理：玩家[`player.gd:21-29`](main/player.gd:21)中实现虚耗层数的获取和1秒自动衰减
...
| **行为模式** | 1. **待机 (Idle):** 站立不动，播放待机动画<br>2. **移动 (Walk):** 生成后直线走向玩家<br>3. **攻击 (Attack):** 到达攻击范围后，执行近战攻击 | 基础的AI行为，专注于分裂机制的威胁构建 |
| **攻击方式** | 近战物理攻击 | 需要玩家在敌人靠近前将其消灭，避免分裂造成数量优势 |
...
| **攻击间隔** | 10秒 | 固定的攻击频率，为玩家提供了计算伤害输出窗口的机会。 |
...
| **行为模式** | 1. **冲锋模式：** 全等级触发，生成后全速直线冲向玩家，进行近战攻击<br>2. **献祭建造模式：** Lv 5 后概率触发<br>&nbsp;&nbsp;&nbsp;&nbsp;① 突进：全速冲至距玩家 150px (贴脸) 处急停<br>&nbsp;&nbsp;&nbsp;&nbsp;② 建造：原地吟唱 4.0秒（两轮动画）<br>&nbsp;&nbsp;&nbsp;&nbsp;③ 结算：若建造结束前未被击杀，则自动死亡 (献祭)，在原地生成一个【白骨图腾】 | 利用高移速干扰防御塔的索敌逻辑，强制吸引火力。 |
| **攻击方式** | 近战物理攻击 | 需要玩家在敌人靠近前将其消灭，避免被贴脸造成威胁 |
| **攻击间隔** | 10秒 | 固定的攻击频率，为玩家提供了计算伤害输出窗口的机会 |
| **移动速度** | 僵尸的1.6倍 | 极高的移动速度使其能够快速穿越战场，干扰防御塔索敌 |
| **实现状态：✅ 已完成（包含献祭机制）**
- ✅ **双模式AI**：`goblin_pioneer.gd` 实现了 `NORMAL`（攻击）和 `BUILDER`（建造）两种模式。
- ✅ **献祭建造**：在 Level 5 后概率触发建造模式，冲至玩家附近吟唱4.0秒（两轮动画）。
- ✅ **距离触发机制**：小于150px距离时100%触发，150-500px范围内线性概率触发。
- ✅ **建造打断**：建造过程中受到攻击会立即打断建造流程。
- ✅ **图腾生成**：吟唱成功后死亡（献祭）并在原地生成【白骨图腾】。
- ✅ **属性特点**：高移动速度，干扰防御塔索敌。
...
| 属性/机制 | 描述 | 设计意图 |
| :--- | :--- | :--- |
| **单位类型** | 地面，肉盾，双刃剑 | 极低生命值（基于等级公式），移动缓慢。受到伤害时不会死亡，而是体型膨胀、颜色变红（过载状态）。 |
| **行为模式** | 1. **待机 (Idle):** 站立不动，体型正常<br>2. **移动 (Walk):** 生成后直线走向玩家，移动缓慢<br>3. **过载状态:** 受到伤害时体型膨胀、颜色变红（[`BoomColorStart`](enemy/glutton.gd:91)）<br>4. **攻击 (Attack):** 到达攻击范围后，执行近战攻击 | 简单的AI行为，专注于过载机制的威胁构建 |
| **攻击方式** | 近战物理攻击（伤害4点）| 需要玩家在敌人靠近前将其消灭，避免接触自爆 |
| **攻击间隔** | 10秒 | 固定的攻击频率，为玩家提供了计算伤害输出窗口的机会 |
| **属性计算** | 生命值：`max(1, int(Level.currentLevel / 3.0))` - 基于关卡数的极低生命值<br>移动速度：基础速度的80% | 生命值随关卡增长但始终较低，移动速度慢 |
| **机制 A** | **过载爆破 (Good)**：触发条件：累计受到的伤害量记录（[`accumulatedDamage`](enemy/glutton.gd:5)）<br>AOE参数：爆炸半径150px，伤害倍率3.0倍（[`aoeDamageMultiplier`](enemy/glutton.gd:10)）<br>效果：对周围敌人造成巨额 AOE 爆炸伤害 (清场)，自身死亡。对玩家无伤。 | 奖励高伤害/爆发流玩家。<br>它相当于一个由玩家亲手引爆的"战术核弹"，是处理后期怪潮的关键手段。 |

**实现状态：🚧 部分完成**
- ✅ **基础行为**：[`glutton.gd`](enemy/glutton.gd:1)实现了移动、受伤、死亡状态
- ✅ **过载视觉**：实现了[`accumulatedDamage`](enemy/glutton.gd:5)记录，受击时通过[`BoomColorStart`](enemy/glutton.gd:91)变红闪烁（红色/白色交替）
- ✅ **属性计算**：生命值使用[`initData`](enemy/glutton.gd:22)中的公式`max(1, int(Level.currentLevel / 3.0))`，速度为基础速度的80%
- ✅ **爆炸参数**：已定义[`aoeExplosionRadius`](enemy/glutton.gd:9)(150px)和[`aoeDamageMultiplier`](enemy/glutton.gd:10)(3.0倍)参数
- ❌ **自爆伤害逻辑**：死亡时的AOE爆炸伤害计算和效果应用尚未实现
- ❌ **过载触发**：累计伤害达到生命值时的自动爆炸机制未实现
...
| **行为模式** | 1. **待机 (Idle):** 站立不动，播放待机动画<br>2. **移动 (Walk):** 生成后直线走向玩家，通常成群结队同步推进<br>3. **攻击 (Attack):** 到达攻击范围后，执行近战攻击 | 集群AI行为，专注于痛苦链接机制的战术配合 |
| **攻击方式** | 近战物理攻击 | 需要玩家在敌人靠近前将其消灭，避免被集群围攻 |
| **攻击间隔** | 10秒 | 固定的攻击频率，为玩家提供了计算伤害输出窗口的机会 |
...
| **行为模式** | 1. **待机 (Idle):** 伪装成场景物体 (树/石)，静止不动<br>2. **移动 (Walk):** 被发现后直线走向玩家<br>3. **攻击 (Attack):** 到达攻击范围后，执行近战攻击<br>4. **幻影机制:** 受击时在原地留下幻影并瞬移隐身 | 复杂的AI行为，专注于欺骗和干扰防御塔系统 |
| **攻击方式** | 近战物理攻击 | 需要玩家在敌人靠近前将其消灭，避免被近身攻击 |
| **攻击间隔** | 10秒 | 固定的攻击频率，为玩家提供了计算伤害输出窗口的机会 |
**实现状态：❌ 未实现**
- 代码中尚未找到对应的实现脚本。
...
| **行为模式** | 1. **待机 (Idle):** 在空中悬浮，触手摆动<br>2. **移动 (Walk):** 生成后缓慢飘向玩家，保持适当距离<br>3. **攻击 (Attack):** 进入射程后持续发射资源吞噬光束 | 飞行单位行为，专注于资源干扰 and 掠夺 |
| **攻击方式** | **弹药吞噬光束**：持续连接玩家，每秒扣除1颗子弹 | 将伤害维度从HP转移到资源，形成独特的威胁模式 |
| **索敌范围** | 全屏射程 (约600-800像素) | 极远的射程使其能够在安全距离进行资源干扰 |
| **攻击间隔** | 1.0秒 (持续伤害) | 高频的资源扣除，持续施加经济压力 |
...
| **行为模式** | 1. **待机 (Idle):** 站立不动，背部水晶微弱发光<br>2. **移动 (Walk):** 生成后直线走向玩家，移动缓慢<br>3. **攻击 (Attack):** 到达攻击范围后，执行近战攻击<br>4. **成长机制:** 附近敌人死亡时吸收灵魂获得充能 | 成长型单位行为，随着战斗进行逐渐变强 |
| **攻击方式** | 近战物理攻击 | 需要玩家在敌人靠近前将其消灭，避免被成长后的强力攻击击中 |
| **攻击间隔** | 10秒 | 固定的攻击频率，为玩家提供了计算伤害输出窗口的机会 |
...
| **行为模式** | 1. **待机 (Idle):** 站立不动，手中法杖散发腐化能量<br>2. **移动 (Walk):** 生成后缓慢移动，保持与玩家的安全距离<br>3. **攻击 (Attack):** 到达适当位置后，为友方单位提供灵魂链接支援 | 辅助型单位行为，专注于为友军提供保护 and 支援 |
| **攻击方式** | **灵魂链接支援**：为最近的友方单位提供伤害转移链接 | 不直接攻击，而是通过链接机制间接参与战斗 |
| **索敌范围** | 中距离范围 (约200-300像素) | 保持安全距离，避免被直接攻击 |
| **攻击间隔** | 1.0秒 (持续链接) | 高频的链接维持，持续提供保护效果 |
...
**代码实现**：`main/UI/answering.gd`
- **功能**：当当前题库题目用尽时，自动暂停游戏并弹出题库选择界面
- **触发条件**：题目数据返回`isNotEnoughWord: true`时触发
- **交互界面**：
  - 半透明黑色背景提示面板
  - 显示所有可用题库的CheckButtons
  - 默认选中当前正在使用的题库
  - "应用并继续游戏"按钮：更新题库配置并恢复游戏
  - "结束游戏"按钮：直接结束当前游戏

**实现状态：✅ 已完成**
- ✅ 自动检测：[`_setTilte`](main/UI/answering.gd:36)中检测题目不足
- ✅ 动态UI：[`_on_word_book_empty`](main/UI/answering.gd:145)创建完整的题库选择界面
- ✅ 题库更新：支持多题库组合选择，调用[`_setWordBookList`](global/jlptn_5.gd:309)更新配置
- ✅ 数据重载：调用[`_gameStart(true)`](global/jlptn_5.gd:573)强制重载题库数据

### 十五、测试模式系统 (Test Mode)

#### 1. 测试模式功能概述
**代码实现**：`main/test.gd`
- **功能**：为开发者和测试者提供的调试功能界面
- **激活条件**：通过 `globalSet.isTestMode` 全局变量控制
- **界面特点**：测试界面可动态显示/隐藏，不影响正式游戏体验

#### 2. 测试模式功能列表
| 功能名称 | 实现代码 | 功能描述 | 使用场景 |
|:--------|:--------|:--------|:--------|
| **玩家升级** | `_on_player_level_up_button_pressed()` | 强制触发玩家升级，测试升级系统 | 测试升级流程和技能选择界面 |
| **添加敌人** | `_on_add_enemy_button_pressed()` | 动态生成各种敌人类型 | 调试特定敌人行为和战斗平衡 |
| **无敌模式** | `_on_invincible_mode_button_pressed()` | 设置玩家生命值和攻击力为99999 | 测试游戏流程和关卡设计 |
| **获取技能** | `_on_get_skill_button_pressed()` | 手动选择并获取任意技能 | 测试技能效果和组合搭配 |
| **增加关卡** | `_on_add_levels_pressed()` | 快速推进关卡进度 | 测试后期内容和难度曲线 |

**实现状态：✅ 已完成**
- ✅ 测试界面：`test.gd` 实现了完整的调试功能界面
- ✅ 功能集成：所有测试功能与游戏核心系统正确集成
- ✅ 动态控制：通过 `globalSet.isTestMode` 实现测试模式开关
- ✅ 开发者友好：提供全面的开发和调试工具

### 十六、Buff系统框架 (Buff System Framework)

#### 1. 通用Buff系统
**代码实现**：`main/buff.gd`
- **功能**：提供视觉Buff效果的基础框架
- **特点**：支持多种Buff的视觉表现和生命周期管理
- **应用场景**：技能效果、状态增益、视觉效果展示

#### 2. 加速Buff系统
**代码实现**：`main/speed_buff.gd`
- **Buff属性**：
  - 速度倍数：`speed_multiplier` (默认1.5倍)
  - 持续时间：`duration` (默认5.0秒)
  - 视觉效果：`buff_color` (默认绿色)
- **技术特点**：
  - 支持Buff应用、叠加、移除的完整生命周期
  - 通过信号系统 (`buffApplied`, `buffRemoved`) 进行状态同步
  - 与敌人Buff系统无缝集成
  - 自动生命周期管理，时间结束自动清理

**实现状态：✅ 已完成**
- ✅ Buff框架：通用Buff系统架构完整实现
- ✅ 视觉系统：支持动态颜色设置和粒子效果
- ✅ 生命周期：完整的Buff应用、持续、移除流程
- ✅ 信号机制：通过信号实现Buff状态变化的通知
- ✅ 兼容性：与敌人Buff系统完美集成

### 十七、Debuff系统 (Debuff System)

#### 1. Debuff视觉效果系统
**代码实现**：`main/debuff.gd`
- **功能**：提供负面状态效果的视觉表现
- **特点**：与Buff系统对应，提供相反方向的视觉效果
- **应用实例**：虚耗精灵的干扰效果视觉展示

#### 2. Debuff系统特点
| 属性/机制 | 数值/说明 | 设计目的 |
|:---------|:---------|:---------|
| **视觉效果** | 向下流动的粒子效果 | 与向上流动的Buff形成视觉对比 |
| **颜色系统** | 支持动态颜色设置 (`SetColor`) | 不同类型Debuff可使用不同颜色区分 |
| **生命周期** | 支持定时器控制 (`SetLiveTime`) | Debuff可设置自动消失时间 |
| **生成控制** | 最多10个Debuff实例同时显示 | 防止视觉效果过于密集 |

**实现状态：✅ 已完成**
- ✅ 视觉系统：完整的Debuff视觉效果实现
- ✅ 颜色管理：支持动态颜色调整
- ✅ 生命周期：自动和手动的生命周期控制
- ✅ 应用实例：虚耗精灵干扰效果的视觉展示

### 十八、音频管理系统 (Audio Management System)

#### 1. 音频管理器架构
**代码实现**：`global/audio_manager.gd`
- **功能**：统一管理游戏中所有音效的播放
- **技术特点**：
  - 预加载音效库，避免运行时加载延迟
  - 支持2D空间音效和全局音效
...
| 音效名称 | 文件路径 | 用途说明 |
|:--------|:--------|:---------|
| **22LRSingleMP3** | `res://sounds/22LRSingleMP3.mp3` | 单发射击音效 |
| **Semi22LRReloadFullMP3** | `res://sounds/Semi22LRReloadFullMP3.mp3` | 完整装弹音效 |
| **Semi22LRCantReloadMP3** | `res://sounds/Semi22LRCantReloadMP3.mp3` | 装弹失败音效 |
| **762x54rSprayIsolatedMP3** | `res://sounds/762x54rSprayIsolatedMP3.mp3` | 连续射击音效 |

#### 3. 音频播放接口
- **全局音效**：`play_sfx(sound_name)` - 适用于UI音效、提示音等
- **空间音效**：`play_sfx_at_position(sound_name, position, volume)` - 适用于位置相关的音效

**实现状态：✅ 已完成**
- ✅ 音效库：预加载所有游戏音效，避免运行时延迟
- ✅ 空间音效：支持2D空间定位的音效播放
- ✅ 内存管理：自动清理已播放的音效节点
- ✅ 音量控制：支持音效的音量调节

### 十九、背景演示系统 (Background Demo System)

#### 1. 背景敌人演示
**代码实现**：`menu/bg_enemy_example.gd`
- **功能**：在菜单界面展示敌人动画效果
- **技术特点**：
  - 随机生成初始位置偏移
  - 随机移动速度和方向
...
| 特性 | 实现方式 | 设计目的 |
|:-----|:---------|:---------|
| **随机化** | `randi_range(50,80)` 随机速度，`randi()%2` 随机方向 | 每次启动菜单都有不同的视觉效果 |
| **位置偏移** | `Vector2(position.x+temp, position.y)` 随机偏移 | 避免敌人重叠，创造更自然的分布 |
| **智能追踪** | `global_position.direction_to(bg_player.global_position)` | 敌人会自动朝向玩家位置移动 |
| **动画系统** | 基于 `AnimatedSprite2D` | 提供流畅的动画表现 |

**实现状态：✅ 已完成**
- ✅ 随机化系统：速度和位置的完全随机化
- ✅ 智能追踪：自动朝向玩家位置的方向调整
- ✅ 动画集成：与AnimatedSprite2D系统完美集成
- ✅ 菜单集成：在主菜单界面提供动态背景效果

### 二十、敌人Buff系统 (Enemy Buff System)

#### 1. 敌人Buff管理框架
**代码实现**：`enemy/base_enemy.gd`
- **功能**：通用的Buff管理框架，支持Buff的应用、叠加、持续时间更新 and 移除。
- **应用实例**：白骨图腾提供的加速Buff。
- **机制**：支持属性倍率（如速度乘区）的实时计算。

#### 3. 敌人状态机系统 (State Machine)
**代码实现**：`enemy/base_enemy.gd`
- **功能**：所有敌人单位统一采用状态机管理行为。
- **状态定义**：
    - `IDLE`: 待机/呼吸
    - `WALK`: 移动追踪玩家
    - `ATT`: 攻击前摇 and 判定
    - `HURT`: 受击硬直
    - `DEATH`: 死亡结算 and 动画
- **优势**：确保了敌人行为逻辑的清晰 and 易扩展性（如哥布林工兵的建造模式即扩展了此状态机）。

#### 4. 玩家无敌机制
**代码实现**：`main/player.gd`
- **受击无敌** (`is_hurt_invincible`)：
    - 触发：受到伤害后。
    - 效果：短暂时间内免疫所有伤害，防止被瞬间秒杀（防暴毙）。
    - 视觉：闪烁红色后恢复。
- **格挡/连击无敌** (`is_parry_invincible`)：
    - 触发：特定连击 or 技能触发（如连答奖励）。
    - 效果：核心逻辑已实现（免疫伤害）。
    - *备注*：代码注释显示该机制的附加效果（如反击、特效）目前标记为"未完成"状态，但不影响基础无敌功能。
