...
**实现状态分析：**
- ✅ N5和N4级别：基础架构已实现，文件路径配置完成
- 🔄 N3级别：路径配置已存在但可能缺少实际数据文件
- ❌ N2和N1级别：尚未实现任何配置
- ❌ 题库解锁机制：尚未实现
...
- ✅ 经验溢出：升级后正确处理溢出经验
- ⚠️ 溢出计算：实际实现中存在计算顺序问题，先减后加可能导致经验损失
...
- ⚠️ 计算顺序：实际实现中先减经验再计算新值，与策划案描述略有差异
...
**实现状态：✅ 已完成**
- ✅ QQ弹（弹射）：[`ricochet_shoot.gd`](skill/scene/ricochet_shoot.gd:16)已实现，支持弹射机制
- ✅ 强力QQ弹：[`ricochet_shoot_up.gd`](skill/scene/ricochet_shoot_up.gd:3)已实现，被动强化弹射
- ✅ 一枪双击：[`double_shoot.gd`](skill/scene/double_shoot.gd:16)已实现，支持层数制
...
| **分裂机制** | 死亡时分裂2-3个小史莱姆 | 每次分裂增加敌人数量，形成指数级威胁增长。 |
| **属性继承** | 小史莱姆继承父史莱姆50%属性 | 保持威胁平衡，避免分裂后过强或过弱。 |
| **分裂次数限制** | 最多分裂1次 | 防止无限分裂，控制战场复杂度。 |
