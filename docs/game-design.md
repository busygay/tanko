...
- **功能实现**：`globalSkillData/totem.gd` 实现了对范围内敌人的检测。
- **效果实现**：为范围内敌人施加持续的加速Buff（代码中目前是加速效果，与策划案的减伤有出入，以代码为准）。
...
- ✅ 层数管理：玩家[`player.gd:19-27`](main/player.gd:19)中实现虚耗层数的获取 and 衰减
...
| **行为模式** | 1. **冲锋模式：** 全等级触发，生成后全速直线冲向玩家，进行近战攻击<br>2. **献祭建造模式：** Lv 5 后概率触发<br>&nbsp;&nbsp;&nbsp;&nbsp;① 突进：全速冲至距玩家 150px (贴脸) 处急停<br>&nbsp;&nbsp;&nbsp;&nbsp;② 建造：原地吟唱 3.0秒<br>&nbsp;&nbsp;&nbsp;&nbsp;③ 结算：若建造结束前未被击杀，则自动死亡 (献祭)，在原地生成一个【白骨图腾】 | 利用高移速干扰防御塔的索敌逻辑，强制吸引火力。 |
...
- ✅ **献祭建造**：在 Level 5 后概率触发建造模式，冲至玩家附近吟唱3秒。
...
| **行为模式** | 1. **待机 (Idle):** 站立不动，体型正常<br>2. **移动 (Walk):** 生成后直线走向玩家，移动缓慢<br>3. **过载状态:** 受到伤害时体型膨胀、颜色变红<br>4. **攻击 (Attack):** 到达攻击范围后，执行近战攻击 | 简单的AI行为，专注于过载机制的威胁构建 |
| **攻击方式** | 近战物理攻击 | 需要玩家在敌人靠近前将其消灭，避免接触自爆 |
| **攻击间隔** | 10秒 | 固定的攻击频率，为玩家提供了计算伤害输出窗口的机会 |
...
**实现状态：🚧 部分完成**
- ✅ **基础行为**：`glutton.gd` 实现了移动、受伤、死亡状态。
- ✅ **过载视觉**：实现了 `accumulatedDamage` 记录，受击时通过 `BoomColorStart` 变红闪烁。
- ✅ **属性特点**：拥有极高的生命值（普通僵尸的4倍）和较低的速度。
- ✅ **爆炸参数**：已定义 `aoeExplosionRadius` (150px) 和 `aoeDamageMultiplier` (3.0倍) 参数。
