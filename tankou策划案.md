...
| 策略 | 触发节拍 | 分岔规则 | 触发题型 |
|----|----------|----------|----------|
| **特殊题型触发** | 每5道普通题后 | 掷0-9：<br>≥5且候选池非空→触发单词重组题型<br>≤4→触发错题消化插入 | 单词重组题型或错题消化插入 |
...
- **优先级排序**：错误次数越高的单词，在后续练习中出现频率越高
- **智能复习**：系统会优先从错题本中选择题目进行强化练习
...
**核心方法**：
- [`_updateErrorWordCount()`](global/jlptn_5.gd:38)：更新单词错误次数
- [`_saveErrorWord()`](global/jlptn_5.gd:89)：保存错题本到本地文件
- [`_loadErrorWord()`](global/jlptn_5.gd:164)：从本地文件加载错题本

**存储机制**：
- **文件路径**：`user://saveErrorWordData.json`
- **数据合并**：新游戏与历史错题数据智能合并，错误次数累加
- **错误处理**：完善的文件读写错误处理机制
...
- **练习模式**：
  ...
  - **随机练习**：从错题本中随机选择题目
  - **重点练习**：只练习错误次数超过阈值的单词
- **进度追踪**：实时显示本次练习的正确率和掌握情况

**强化机制**：
- **错误重置**：练习中答对的单词，错误次数-1
- **连续答对奖励**：连续答对同一单词3次，额外-1错误次数
- **练习完成**：当单词错误次数降至0时，显示"已掌握"标记

#### 3. 错题统计分析

**统计图表**：
- **错误分布图**：按JLPT级别显示错误单词分布
- **学习进度图**：显示错题总数随时间的变化趋势
- **掌握情况图**：显示已掌握/未掌握单词的比例

**学习报告**：
- **日/周/月报告**：定期生成学习进度报告
- **薄弱环节分析**：识别最容易出错的词汇类型
- **学习建议**：基于错题数据提供个性化学习建议
...
**技术实现**：
- 跳过按钮通过 [`skip_button.pressed`](main/UI/answering.gd:22) 信号连接 [`_on_skip_pressed()`](main/UI/answering.gd:105) 方法
- 触发 [`Eventmanger.questionSkipped.emit()`](global/eventmanger.gd:93) 信号通知游戏系统
- 直接调用 [`_settilte()`](main/UI/answering.gd:31) 加载新题目，无资源变化

**游戏平衡考虑**：
- 跳过功能无使用次数限制，但频繁跳过会导致资源获取效率低下
- 在高难度关卡中，合理使用跳过功能是重要的策略选择
- 跳过不会影响错题本的统计，确保错题本的准确性
...
#### 3. 虚耗精灵（Drain Sprite）（待开发）

虚耗精灵是一种特殊的干扰型敌人，不直接造成伤害，但会给玩家施加持续的资源获取干扰效果。

| 属性/机制 | 描述 | 设计意图 |
| :--- | :--- | :--- |
| **单位类型** | 特殊，干扰 | 不直接威胁玩家生命值，但影响资源获取效率 |
| **攻击方式** | 虚耗攻击 | 不扣血，给玩家+1「虚耗层」（上限10层） |
| **干扰机制** | EP获取干扰 | 每次获得EP时，每层独立5%概率使该次EP-1；若触发，则层数立即÷2（向下取整） |
| **清层方式** | 多种清除机制 | 1. 消耗1AP技能「净化脉冲」瞬间清零<br>2. 自然衰减（成功-1EP时半清）<br>3. 关卡失败/重开强制清零 |
| **视觉提示** | 淡紫色层数提示 | 题干零改动，仅在UI上显示淡紫色层数提示 |
| **刷出规则** | 20关后限制出现 | 20关后，每5关最多1只；击杀+5Bullet（无EP） |
| **战略价值** | 资源管理挑战 | 迫使玩家在资源积累和干扰清除之间做出战略选择 |
...
2. **干扰计算**：
   - 每次获得EP时，系统为每层虚耗层独立进行5%概率判定
   - 若任意一层触发干扰，则该次EP获取量-1
   - 干扰触发后，虚耗层数立即减半（向下取整）

3. **清除策略**：
   - **主动清除**：消耗1AP使用「净化脉冲」技能，瞬间清零所有虚耗层
   - **被动清除**：当玩家成功减少1EP时（如答错题目），虚耗层数减半
   - **强制清除**：关卡失败或重新开始时，虚耗层强制清零

**设计哲学**：
虚耗精灵的设计旨在引入一种非直接伤害的资源管理挑战，迫使玩家在追求高效资源获取和应对干扰之间做出战略权衡。这种机制增加了游戏的深度，使玩家不仅要关注战场上的直接威胁，还要管理持续的资源获取效率。
